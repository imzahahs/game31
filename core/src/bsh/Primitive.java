...
*/
@DefaultSerializer(value = Primitive.PrimitiveSerializer.class)
public final class Primitive implements ParserConstants, java.io.Serializable
{
	public static class PrimitiveSerializer implements Serializer<Primitive> {
		...
		static final byte DOUBLE = 9;

		@Override
		public Primitive read(Mass m, Input s, Class<Primitive> clazz) {
			...
			if(type == NULL)
				...
				return new Primitive(s.readBoolean());
			else if(type == BYTE)
				return new Primitive(s.readByte());
			else if(type == SHORT)
				return new Primitive(s.readShort());
			else if(type == CHAR)
				return new Primitive(s.readChar());
			else if(type == INT)
				return new Primitive(s.readInt());
			else if(type == LONG)
				return new Primitive(s.readLong());
			else if(type == FLOAT)
				return new Primitive(s.readFloat());
			else // if(type == DOUBLE)
				return new Primitive(s.readDouble());
		}

		@Override
		public void write(Mass m, Output s, Primitive o) {
			if(o.value == Special.NULL_VALUE)
				s.writeByte(NULL);
			else if(o.value == Special.VOID_TYPE)
				s.writeByte(VOID);
			else if(o.value.getClass() == Boolean.class) {
				s.writeByte(BOOLEAN);
				s.writeBoolean((Boolean)o.value);
			}
			else if(o.value.getClass() == Byte.class) {
				s.writeByte(BYTE);
				s.writeByte((Byte)o.value);
			}
			else if(o.value.getClass() == Short.class) {
				s.writeByte(SHORT);
				s.writeShort((Short)o.value);
			}
			else if(o.value.getClass() == Character.class) {
				s.writeByte(CHAR);
				s.writeChar((Character)o.value);
			}
			else if(o.value.getClass() == Integer.class) {
				s.writeByte(INT);
				s.writeInt((Integer)o.value);
			}
			else if(o.value.getClass() == Long.class) {
				s.writeByte(LONG);
				s.writeLong((Long)o.value);
			}
			else if(o.value.getClass() == Float.class) {
				s.writeByte(FLOAT);
				s.writeFloat((Float)o.value);
			}
			else { // if(primitive.value.getClass() == Double.class)
				s.writeByte(DOUBLE);
				s.writeDouble((Double)o.value);
			}
		}
	}
